...
def phong_illuminate(scene, intersection, viewer):
    """Return the intensity of the light at intersection from viewer with the Phong reflection model"""
    ...
    #Constants of specular, diffuse and ambient reflection, and shininess constant
    n = normalize(intersection.normal)
    l = [normalize(scene.lights[i].position -
                   intersection.position) for i in range(nblights)]
    #l contain the direction vectors from the intersection toward each light source
    r = [normalize(2 * (l[i].dot(n)) * n - l[i]) for i in range(nblights)]
    #r contain the directions that a perfectly reflected ray of light would take
